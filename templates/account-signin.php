<?php if (is_user_logged_in()) : ?>
	<div class="wpfm-form-group ">
		...
		<div class="field account-sign-in wpfm-alert wpfm-alert-info"> <?php $user = wp_get_current_user();
																																		printf(wp_kses(__('You are currently signed in as <strong>%s</strong>.', 'wp-food-manager'), array('strong' => array())), $user->user_login);    		?> <a href="<?php echo apply_filters('submit_food_form_logout_url', wp_logout_url(get_permalink())); ?>"><?php _e('Sign out', 'wp-food-manager'); ?></a>
		</div>
	</div>
<?php else :
	...
	<div class="wpfm-form-group">
		...
		<div class="field account-sign-in wpfm-alert wpfm-alert-info">
			<a href="<?php echo !empty(get_option('food_manager_login_page_url')) ? esc_url(apply_filters('submit_food_form_login_url', get_option('food_manager_login_page_url'))) : 	home_url() . '/wp-login.php'; ?>"><?php _e('Sign in', 'wp-food-manager'); ?></a>
			...
			<?php elseif ($account_required) : ?>
				<?php echo  wp_kses_post(apply_filters('submit_food_form_login_required_message',  __(' You must sign in to create a new listing.', 'wp-food-manager'))); ?>
			<?php endif; ?>
		</div>
	</div>
	<?php if ($registration_enabled) :
		if (!empty($registration_fields)) {
			foreach ($registration_fields as $key => $field) {			?>
				<div class="wpfm-form-group fieldset-<?php echo esc_attr($key); ?>">
					<label class="wpfm-form-label-text" for="<?php echo esc_attr($key); ?>"><?php echo esc_html($field['label']) . apply_filters('submit_food_form_required_label', $field['required'] ?   '<span class="require-field">*</span>' : ' <small>' . __('(optional)', 'wp-food-manager') . '</small>', $field); ?></label>
