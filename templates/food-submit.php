	...
	<?php if (wpfm_user_can_post_food() || food_manager_user_can_edit_food($food_id)) : ?>
		...
		foreach ($food_fields as $key => $field) :
			...
			<label for="<?php echo esc_attr($key); ?>" class="wpfm-form-label-text">
				<?php
				echo esc_html($field['label']) . wp_kses_post(
					apply_filters(
						'add_food_required_label',
						(!empty($field['required']) ? '<span class="require-field">*</span>' : ' <small>' . esc_html(__('(optional)', 'wp-food-manager')) . '</small>'),
						$field
					)
				);
				?>
		...
		if ((isset($_POST['food_id']) && !empty($_POST['food_id'])) || (isset($_GET['action']) == 'edit')) {
			if ($topping_fields) : ?>
				...
				<h3 class="wpfm-form-title wpfm-heading-text"><?php esc_html_e('Extra Toppings', 'wp-food-manager'); ?></h3>
				<div class="wpfm-options-wrapper wpfm-metaboxes">
					<?php if (!empty($extra_fields_options)) {
						foreach ($topping_items as $key => $extra_fields_option) {
							...
							<div class="wpfm-options-wrap wpfm-metabox postbox wpfm-options-box-<?php echo esc_attr($key); ?>">
								<input type="hidden" name="repeated_options[]" value="<?php echo esc_attr($key); ?>" class="repeated-options">
								<h3 class="">
									<a href="javascript: void(0);" data-id="<?php echo esc_attr($key); ?>" class="wpfm-delete-btn dashicons dashicons-dismiss">Remove</a>
									<div class="wpfm-togglediv" title="Click to toggle" aria-expanded="false" data-row-count="<?php echo esc_attr($key); ?>"></div>
									<div class="wpfm-sort"></div>
									<strong class="attribute_name"><?php echo esc_html($extra_fields_option['_topping_name']); ?></strong>

									<span class="attribute_key"><input type="hidden" name="topping_key_<?php echo esc_attr($key); ?>" value="<?php echo esc_html($topping_key); ?>" readonly=""></span>
								</h3>
									...
									<div class="wpfm-content">
										...
										foreach ($topping_fields as $topping_field_key => $field) :
											if ($topping_field_key !== 'topping_options') {
												...
												<fieldset class="wpfm-form-group fieldset<?php echo esc_attr($topping_field_key); ?> <?php echo esc_attr($fieldClassLabel); ?>" data-field-name="<?php echo esc_attr($topping_field_key); ?>">
												<label class="wpfm-form-label-text">
													<?php
													echo esc_html($field['label']) . wp_kses_post(
														apply_filters(
															'add_food_required_label',
															(!empty($field['required']) ? '<span class="require-field">*</span>' : ' <small>' . esc_html__('(optional)', 'wp-food-manager') . '</small>'),
															$field
														)
													);
													?>
											...
											<?php }
											if ($topping_field_key == 'topping_options') { ?>
												<fieldset class="wpfm-form-group fieldset_topping_options_<?php echo esc_attr($key); ?> ">
													<label><?php esc_html_e('Options', 'wp-food-manager');?> <small><?php esc_html_e('(optional)', 'wp-food-manager');?></small></label>
													<div class="field ">
														<table class="widefat">
															<thead>
																<tr>
																	<th> </th>
																	<th>#</th>
																	<th><?php esc_html_e('Label', 'wp-food-manager');?></th>
															...
															<tbody class="ui-sortable">
																...
																foreach ($extra_fields_option['_topping_options'] as $sub_value_count => $values) {
																	...
																	<tr class="option-tr-<?php echo esc_attr($sub_value_count); ?>">
																		<td><span class="wpfm-option-sort">☰</span></td>
																		<td><?php echo esc_attr($sub_value_count); ?></td>
																		<td>
																			<input type="text" name="<?php echo esc_attr($key); ?>_option_name_<?php echo esc_attr($sub_value_count); ?>" value="<?php echo esc_attr($values['option_name']); ?>" class="opt_name" pattern=".*\S+.*" required>
																		</td>
												...
												"><?php esc_html_e('Add Option', 'wp-food-manager'); ?></a>
																	</td>
																</tr>
															</tfoot>
														</table>
					...
					<div class="wpfm-actions">
						...
						<div class="wpfm-options-wrap wpfm-metabox postbox wpfm-options-box-__repeated-option-index__">
						        ...
						        <h3 class="">
						            ...
						            <div class="wpfm-sort"></div>
						            <strong class="attribute_name"><?php esc_html_e("Topping Option __repeated-option-index__", "wp-food-manager"); ?></strong>
