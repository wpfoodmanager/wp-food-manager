...
<div class="wpfm-main wpfm-single-food-menu-page wpfm-accordion-body">
    ...
    <?php } ?>
    <div class='fm-food-menu-container'>
        <?php $price_decimals = wpfm_get_price_decimals();
        ...
        $menu_food_desc = '';
        $sale_price = get_post_meta(get_the_ID(), '_food_sale_price', true);
        $regular_price = get_post_meta(get_the_ID(), '_food_price', true);
        ...
        if( $food_redirect_option == 'yes'){
            ...
            $food_menu_permalink = esc_url(get_permalink(get_the_ID()));
        ...
        <div class='food-list-box' data-id='<?php echo get_the_ID();?>'>
            ...
            <?php if ($thumbnail_option != 'thumbnail_disabled' && $show_image) {
            ...
            <div class='wpfm-food-list-box-content-col'>
                <?php if (!empty($food_label)) {
                    echo "<div class='food-menu-label'>" . $food_label . "</div>";
                }
                echo "<a $food_menu_return_false href='" . $food_menu_permalink . "'>"; ?>
                    <div class='wpfm-food-menu-title-container'>
                        <h3 class='fm-food-menu-title'> 
                            <?php echo esc_html(get_the_title()); ?> 
                            <?php wpfm_display_food_veg_nonveg_icon_tag(get_the_ID());?>
                        </h3>
                        <?php do_action('food_menu_list_title_before',get_the_ID()); ?>
                    </div>
                    <div class='fm-food-menu-pricing'>
                ...
                <div class='fm-food-menu-description'>
                    <?php echo $menu_food_desc;?>
                </div>
                <?php if (wpfm_get_stock_status(get_the_ID()) == 'food_outofstock') {
                    echo '<div class="food-stock-status">';
                        wpfm_display_stock_status(get_the_ID());
                    echo '</div>';
                }
                do_action('food_menu_list_overview_after', get_the_ID());?>
            </div>
        </div>
        <?php do_action('food_menu_list_end'); ?>
    </div>
</div>

<!-- FOOD POPUP HTML -->
<div id="wpfm_food_popup" class="wpfm-modal wpfm-food-popup" role="dialog" aria-labelledby="Food" style=""></div>
