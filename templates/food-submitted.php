...
switch (esc_attr($food->post_status)):
	case 'publish':
		...
		printf('<p class="post-submitted-success-green-message wpfm-alert wpfm-alert-success">' . esc_html__('%1$s listed successfully. To view your listing <a href="%2$s">click here</a>.', 'wp-food-manager') . '</p>', 
	...
	case 'pending':
		...
		printf('<p class="post-submitted-success-green-message wpfm-alert wpfm-alert-success">' . esc_html__("Your %s has been successfully added to WP Food Manager. Your food listing will be visible after the admin's approval.", 'wp-food-manager') . '</p>', esc_html($wp_post_types['food_manager']->labels->singular_name), esc_url(get_permalink($food->ID)));
		break;
	default:
