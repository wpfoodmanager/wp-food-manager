...
switch ($food->post_status):
	case 'publish':
		...
		break;
	case 'pending':
		printf('<p class="post-submitted-success-green-message wpfm-alert wpfm-alert-success">' . __("Your %s has been successfully added to WP Event Manager. Your food listing will be visible after the admin's approval.", 'wp-food-manager') . '</p>', $wp_post_types['food_manager']->labels->singular_name, get_permalink($food->ID));
		break;
	default:
