...
switch (esc_attr($food->post_status)):
	case 'publish':
		printf('<p class="post-submitted-success-green-message wpfm-alert wpfm-alert-success">' . __('%s listed successfully. To view your listing <a href="%s">click here</a>.', 'wp-food-manager') . '</p>', esc_html($wp_post_types['food_manager']->labels->singular_name), esc_url(get_permalink($food->ID)));
		break;
	case 'pending':
