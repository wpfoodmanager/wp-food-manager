<?php if (!empty($field['value']) && is_array($field['value'])) : ?>
                        ...
                        <div class="row">
                            <fieldset class="wpfm-form-group fieldset-<?php echo esc_attr($subkey); ?>">
                                <?php if (!empty($subfield['label'])) : ?>
                                    <label for="<?php echo esc_attr($subkey); ?>" class="wpfm-form-label-text"><?php echo esc_html(sanitize_title($subfield['label'])) . ($subfield['required'] ? '' : ' <small>' . __('(optional)', 'wp-food-manager') . '</small>'); ?></label>
                                <?php endif; ?>
                                <div class="field">
                                    <?php
    ...
    <?php endforeach; ?>
<?php endif; ?>
<button class="button"><?php _e('Add field', 'wp-food-manager'); ?></button>
<a href="#" class="wpfm-theme-text-button wpfm_repeated_add_field" data-row="<?php echo esc_attr(ob_start()); ?>
    ...
    <div class="wpfm-tabs-wrapper wpfm-add-tab-wrapper">
        ...
        <ul class="wpfm-tabs-wrap">
            ...
            <li class="wpfm-tab-link" data-tab="<?php echo esc_attr($key); ?>_%%repeated-row-index%%"><?php _e('Settings', 'wp-food-manager'); ?></li>
        </ul>
        <div id="<?php echo esc_attr($key . '-' . '%%repeated-row-index%%'); ?>" class="wpfm-tab-content current">
            <div id="%%repeated-row-index%%" class="wpfm-tab-pane active">
                <?php foreach ($field['fields'] as $subkey => $subfield) : ?>
                    <fieldset class="wpfm-form-group fieldset-<?php echo esc_attr($subkey); ?>">
                        <?php if (!empty($subfield['label'])) : ?>
                            <label for="<?php echo esc_attr($subkey); ?>" class="wpfm-form-label-text"><?php echo esc_html($subfield['label']) . ($subfield['required'] ? '' : ' <small>' . __('(optional)', 'wp-food-manager') . '</small>'); ?></label>
                        <?php endif; ?>
                        <div class="field">
                            <?php
