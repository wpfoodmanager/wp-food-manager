                ...
                jQuery(this).autocomplete({
                    ...
                    select: function (event, ui) {
                        ...
                        if (ui.item.selection_type != '') {
                            wrapper.find('[name="_option_type_' + count + '"]').val(ui.item.selection_type);
                        } else {
                            wrapper.find('[name="_option_type_' + count + '"]').val('');
                        }
                        if (ui.item.required != '') {
                            wrapper.find('[name="_option_required_' + count + '"][value="'+ui.item.required+'"]').prop('checked', true);
                        } else {
                            wrapper.find('[name="_option_required_' + count + '"]').prop('checked', false);
                        }
                        if (ui.item.description != '') {
                            tinyMCE.get('_option_description_' + count).setContent(ui.item.description);
                        } else {
                            tinyMCE.get('_option_description_' + count).setContent('');
                        }
                    },
                });
            });
