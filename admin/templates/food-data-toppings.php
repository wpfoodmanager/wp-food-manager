		...
		<div class="wpfm-options-wrapper wpfm-metaboxes">
			...
			<?php
				...
				<div class="wpfm-options-wrap wpfm-metabox postbox wpfm-options-box-1">
					<input type="hidden" name="repeated_options[]" value="1" class="repeated-options">
					<h3 class="">
						<a href="javascript: void(0);" data-id="1" class="wpfm-delete-btn">Remove</a>
					...
					<span class="attribute_key"><input type="hidden" name="topping_key1"
							value="option1" readonly>
					</span>
				</h3>
					...
					<div class="wpfm-content">
						...
						if (isset($topping_fields["toppings"]))
							foreach ($topping_fields["toppings"] as $key => $field) {
								...
								if ($type == "wp-editor")
									$type = "wp_editor";
								if ($type == "term-autocomplete")
									$type = "term_autocomplete";
								?>
								<p class="wpfm-admin-postbox-form-field <?php echo esc_attr($key);
								echo ($type == "wp_editor") ? ' wp-editor-field' : ''; ?>" <?php echo ($type == "wp_editor") ? 'data-field-name="' . esc_attr($key) . '"' : ''; ?>>
									<label for="<?php echo esc_attr($key); ?>"><?php echo esc_html($field['label']); ?> : </label>
								...
								<?php endif; ?>
								<?php if ($type != 'options')
									echo '<span class="wpfm-input-field">'; ?>
								<?php get_food_manager_template('form-fields/' . $field['type'] . '-field.php', array('key' => esc_attr($key), 'field' => $field)); ?>
								<?php if ($type != 'options')
									echo '</span>'; ?>
								</p>
