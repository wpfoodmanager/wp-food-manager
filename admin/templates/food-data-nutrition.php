 ...
 */
?>
<div id="nutritions_food_data_content" class="panel wpfm_panel wpfm-metaboxes-wrapper">
	<div class="wp_food_manager_meta_data">

		<?php do_action('food_manager_food_data_nutrition_start', $thepostid);
		...
		<div class="wpfm-nutrition-fields wpfm-metaboxes">
			<p class="wpfm-nutrition-notice">
				<?php _e('Please drag and drop the available nutritions and enter the value.', 'wp-food-manager'); ?>
			</p>
						...
						foreach ($meta_nutritions as $nutritions) {
							foreach ($nutritions as $nutrition) {
								...
								if ($nutrition_term_id) { ?>
									<li class='wpfm-sortable-item active-item' data-id="<?php esc_attr_e($nutrition_term_id);?>">
										<label><?php esc_html_e($nutrition_term_name);?></label>
										<div class='wpfm-sortable-item-values'>
											<input type='number' step='any' class='item-value' name='food_nutritions[<?php esc_attr_e($nutrition_term_id);?>][value]' value='<?php esc_attr_e($nutrition_value);?>'>
											<select name='food_nutritions[<?php esc_attr_e($nutrition_term_id);?>][unit_id]' class='item-unit'>
												<option value=''><?php _e('Unit', 'wp-food-manager');?></option>
												<?php if (!empty($units)) {
													foreach ($units as $unit) {
														$sel = ($unit_id == $unit->term_id ? ' selected' : null);
														echo "<option value='" . esc_attr($unit->term_id) . "'" . ( !empty($sel) ? ' ' . esc_attr($sel) : '') . ">" . esc_html($unit->name) . "</option>";
													}
												} ?>
											</select>
										</div>
				...
				<ul id="wpfm-available-nutri-list" class="wpfm-available-list wpfm-sortable-list wpfm-clear ui-sortable" data-title="Available Nutrition">
					<li class="wpfm-item-search with-title">
						<label class="wpfm-search-label">
							<span><?php esc_html_e('Search Nutrition', 'wp-food-manager')?></span>
					...
					<li class="wpfm-no-results" style="display: none;">
						<?php esc_html_e('No results found', 'wp-food-manager'); ?>
					</li>
				</ul>
			</div>
	...
	<?php do_action('food_manager_food_data_nutrition_end', $thepostid); ?>
	</div>
</div>