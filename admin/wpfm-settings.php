				...
				'food_listings'       => array(
					...
					array(
						...
						array(
							...
							'desc'       => __('If enabled, the Food Category select box will default to a multi select on the [foods] shortcode.', 'wp-food-manager'),
							'type'       => 'checkbox',
							'attributes' => array(),
						),
						array(
						...
						foreach ($this->settings as $key => $section) {
							...
							foreach ($section[1] as $option) {
								...
								switch ($option['type']) {
									case "checkbox": ?>
										<label>
											<input id="setting-<?php echo $option['name']; ?>" name="<?php echo $option['name']; ?>" type="checkbox" value="1" <?php echo implode(' ', $attributes); ?> <?php checked('1', $value); ?> /> <?php echo $option['cb_label']; ?>
										</label>
									<?php
										if ($option['desc'])
